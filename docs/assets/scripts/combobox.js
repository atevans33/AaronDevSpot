...
//save a list of element actions for future readability
const SelectActions = {
    Close: 0,
    CloseSelect: 1,
    First: 2,
...
function filterOptions(options = [], input, exclude = []) {
...
*/Select
function Select (el, options = []) {//define as class
    //element refs
    this.el = el;
...
Select.prototype.init = function() {
    ...
    this.comboEl.innerHTML = this.options[3];
    ...
    this.options.map((option, index) => {
        const optionsEl = this.createOption(option, index);
        this.listboxEl.appendChild(optionsEl);
    });
};
...
Select.prototype.createOption = function (optionText, index) {
    ...
    optionsEl.setAttribute('role','option');
    optionsEl.id = `${this.idBase}-${index}`;//template literal utilizing backquotes
    optionsEl.className = index === 3 ? 'combo-option option-current' : 'combo-option';
    optionsEl.setAttribute('aria-selected', `${index === 3}`);
    optionsEl.innerText = optionText;

    optionsEl.addEventListener('click', (event) => {
        event.stopPropagation();
        this.onOptionClick(index);
    });
    optionsEl.addEventListener('mousedown', this.onOptionMouseDown.bind(this));
